...
function datatext() {
  ...
  chart.render = function (container, calc, dv) {
    ...
    if (isString(state.height) || isNumber(state.height)) {
      calc.pre
        .style("height", append$1(state.height, "px"))
 ...
 */
function append$1(string, appendix) {
 ...
 */
function svgExport(selector, filename) {
...
export { DataController, DataItem, Dataset, date_ordinator as DateOrdinator, bar, LOTIVIS_CONFIG as config, csv, csvParse, csvRender, index as d3, datatext, debug, download, json, jsonFlat, legend, map, parseDataset, parseDatasets, plot, svgExport as svgDownload, toDataset };
