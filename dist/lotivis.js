...
Color$1.colorsForStack = function (stackNumber, amount = 1) {
  ...
  let colorGenerator = linear().domain([0, amount]).range([colorCouple[0], colorCouple[1]]);
  let colors = [];
...
class DateChartAxisRenderer {
  ...
  constructor(dateChart) {
    ...
    this.render = function () {
      ...
      dateChart.svg.append("g").call(axisLeft(dateChart.yChart)).attr("transform", () => `translate(${margin.left},0)`); // bottom axis

      dateChart.svg.append("g").call(axisBottom(dateChart.xChartScale)).attr("transform", () => `translate(0,${height - margin.bottom})`);
    };
  }
...
class TimeChartLegendRenderer {
  constructor(dateChart) {
    ...
    function renderDefaultLegend() {
      ...
      let circleRadius = 6;
      let labelMargin = 50;
      let xLegend = band().domain(datasetNames).rangeRound([config.margin.left, config.width - config.margin.right]);
    ...
    function renderCombinedStacksLegend() {
      ...
      let circleRadius = 6;
      let labelMargin = 50;
      let xLegend = band().domain(stackNames).rangeRound([dateChart.config.margin.left, dateChart.config.width - dateChart.config.margin.right]);
  ...
  constructor(mapChart) {
    ...
    this.render = function () {
      ...
      for (let index = 0; index < stackNames.length; index++) {
        ...
        let max = max$3(dataForStack, item => item.value) || 0;
        let offset = index * 80;
  ...
  constructor(mapChart) {
    ...
    this.render = function () {
      ...
      for (let index = 0; index < stackNames.length; index++) {
        let stackName = stackNames[index];
        let dataForStack = combinedData.filter(data => data.stack === stackName);
        let max = max$3(dataForStack, item => item.value);
...
class MapChart extends Chart {
  ...
  initialize() {
    ...
    this.path = index$2().projection(this.projection);
    this.initializeRenderers();
  }
  ...
  geoJSONDidChange() {
    ...
    this.geoJSON.features.forEach(feature => feature.center = centroid$1(feature));
...
class PlotAxisRenderer {
  ...
  constructor(plotChart) {
    ...
    this.renderAxis = function () {
      ...
      plotChart.svg.append("g").call(axisTop(plotChart.xChart)).attr("transform", () => `translate(0,${margin.top})`); // left

      plotChart.svg.append("g").call(axisLeft(plotChart.yChart)).attr("transform", () => `translate(${margin.left},0)`); // bottom

      plotChart.svg.append("g").call(axisBottom(plotChart.xChart)).attr("transform", () => `translate(0,${plotChart.height - margin.bottom})`);
    };
  }
...
DatasetsController.prototype.getDateDataview = function (groupSize) {
  ...
  dataView.max = max$3(dataView.datasetStacksPresented, function (stack) {
    return max$3(stack, function (series) {
      return max$3(series.map(item => item["1"]));
    });
  });
